...
# -----------------------------
project_root = Path(__file__).resolve().parent.parent  # sentiment-analysis/
...
def load_predictor():
    logger.info("Loading SentimentPredictor for Streamlit...")
    return SentimentPredictor(model_dir="checkpoints/best_model")

predictor = load_predictor()

text_input = st.text_area("Enter text (Sinhala or English)", height=150)

if st.button("Predict"):
    if not text_input.strip():
        st.warning("Please enter some text.")
    else:
        preds, probs = predictor.predict(text_input)
        pred = preds[0]
        prob = probs[0]
